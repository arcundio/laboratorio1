package co.edu.uniquindio.barberiavip.utils;
...
public class FiltroToken extends OncePerRequestFilter {
    ...
    public void doFilterInternal(HttpServletRequest req, HttpServletResponse res, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (req.getMethod().equals("OPTIONS")) {
            ...
            try {
                if (requestURI.startsWith("/api/clientes") || requestURI.startsWith("/api/administrador")) {
                    if (token != null) {
                        Jws<Claims> jws = jwtUtils.parseJwt(token);
                        if (
                                (requestURI.startsWith("/api/clientes") &&
                                        !jws.getBody().get("rol").equals("cliente")) ||
